...
const titulo = ref("");
const subtitulo_1 = ref("");
const subtitulo_2 = ref("");
const subtitulo_3 = ref("");
...
const fetchAboutUs = async () => {
  try {
    ...
    titulo_seccion.value = nosotros.value.titulo_seccion || {};
    titulo.value = titulo_seccion.value.titulo || "";
    subtitulo_1.value = titulo_seccion.value.subtitulo_1 || "";
    subtitulo_2.value = titulo_seccion.value.subtitulo_2 || "";
    subtitulo_3.value = titulo_seccion.value.subtitulo_3 || "";
    // console.log(titulo_seccion.value);
  } catch (error) {
...
<template>
  <div class="about-area py-16 bg-white" v-if="nosotros && nosotros.imagen">
    ...
    >
      <ImageWithFrames
        :imageUrl="`${baseApiUrl}${nosotros.imagen.url}`"
        :alt="nosotros.imagen.name || 'Imagen de nosotros'"
        :aboutUs="true"
        :description="nosotros.mensaje_imagen"
        greenBorderClass="absolute -top-4 -left-4 xs:-top-4 xs:-left-4 sm:-top-8 sm:-left-8 md:-top-10 md:-left-10 lg:-top-8 lg:-left-8 w-[200px] h-[260px] xs:w-[200px] xs:h-[290px] sm:w-[250px] sm:h-[430px] md:w-[300px] md:h-[360px] lg:w-[300px] lg:h-[440px] border-[10px] sm:border-[15px] md:border-[17px] border-[#2ebaa1] rounded-[15px] sm:rounded-[20px] lg:rounded-[25px] z-0"
      ...
      <div class="lg:w-1/2 lg:px-0 text-center lg:text-left">
        <div>
          ...
          >
            <div>
              <div
                v-for="beneficio in nosotros.beneficios"
                :key="beneficio.id"
              ...
              >
                <div class="feature-icon flex justify-left mr-4">
                  ...
                  >
                    <img
                      :src="`${baseApiUrl}${beneficio.icono.url}`"
                      :alt="beneficio.icono.name"
                ...
                <div>
                  <h5
                    class="font-bold text-sm sm:text-base md:text-lg mb-1 sm:mb-2"
                  >
                    {{ beneficio.titulo }}
                  </h5>
